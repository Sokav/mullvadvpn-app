...
class ProblemReportViewController: UIViewController, UITextFieldDelegate, ConditionalNavigation {
    ...
    override func viewDidLoad() {
        ...
        textViewKeyboardResponder = AutomaticKeyboardResponder(targetView: messageTextView, handler: { (textView, adjustment) in
            let adjustment = textView.isFirstResponder ? adjustment : 0

            textView.contentInset.bottom = adjustment
            textView.scrollIndicatorInsets.bottom = adjustment
        })

        scrollViewKeyboardResponder = AutomaticKeyboardResponder(targetView: self.scrollView, handler: { (scrollView, adjustment) in
